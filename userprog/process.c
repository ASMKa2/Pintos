...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  char*tmp;
  char*tok = strtok_r(copied_file_name, " ", &tmp);

  int argc;
  for(argc = 0; tok != NULL; argc++){
  ...
  tok = strtok_r(copied_file_name, " ", &tmp);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      /*insert vm_entry to hash table vm */
      ASSERT(vm_insert_entry(&thread_current()->vm, vme) == true);
...
setup_stack (void **esp) 
{
  ...
  /* insert vm_entry to hash table vm */
  success = vm_insert_entry(&thread_current()->vm, vme);
  return success;
}
...
bool expand_stack(void *addr){
  ...
  /* check if more memory for stack is available */
  if((size_t)(PHYS_BASE - pg_round_down(addr)) >= MAX_STACK_SIZE){
    exit(-1);
  }

  p = alloc_page(PAL_USER);
